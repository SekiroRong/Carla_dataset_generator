...
from agents.navigation.behavior_types import Cautious, Aggressive, Normal, GOGOGO
...
class BehaviorAgent(BasicAgent):
    ...
    def __init__(self, vehicle, behavior='normal'):
        ...
        elif behavior == 'GOGOGO':
            self._behavior = GOGOGO()
    ...
    def run_step(self, debug=False):
        ...
        if self._behavior.obey_traffic_light:
            if self.traffic_light_manager():
                self.isStop = True
                return self.emergency_stop()
        # else:
        #     if self.traffic_light_manager():
        #         print("GOGOGO: Breaking Traffic Light!")
        ...
        if vehicle_state:
            ...
            # Stop Recording if the car is very very close.
            if distance < 0.5 * self._behavior.braking_distance:
                self.isStop = True
            # Emergency brake if the car is very close.
            if distance < self._behavior.braking_distance:
                return self.emergency_stop()
